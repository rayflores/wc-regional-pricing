...
class WCRP_Admin {
	...
	public function wcrp_modify_cart_display_taxes( $tax_totals, $cart ){
		...
		$discount_amount = 0;
		if (count($cart->applied_coupons) > 0){
			foreach ($cart->applied_coupons as $coupon_code){
				//print_r($coupon_code);
				$discount_amount += $cart->get_coupon_discount_amount( $coupon_code, true );
			}
		} 
		$cart_items_totals = $total_items_total + $reg_item_subtotal - $discount_amount;
		//print_r($cart->taxes); // 12.22
		foreach ($item_tax_rates as $tax ){
			$rate = $tax['rate'];
		}
		$tax_rate = ($rate / 100);
		$total_taxes = $cart_items_totals * $tax_rate;
		// not needed
		//$cart->tax_total = $total_taxes;
		...
		$tax_totals = array();
			...
			foreach ( $taxes as $key => $tax ) {
				...
				if ( $code || $key === apply_filters( 'woocommerce_cart_remove_taxes_zero_rate_id', 'zero-rated' ) ) {
					...
					$tax_totals[ $code ]->amount           += wc_round_tax_total( $total_taxes ); // here
	...
	public function wcrp_modify_cart_totals( $cart ){
		...
		$cart_items_totals = $total_items_total + $reg_total_items_total - $discount_amount;
		//print_r($cart->taxes); // 12.22
		foreach ($item_tax_rates as $tax ){
			$rate = $tax['rate'];
		}
		...
		$cart->tax_total = $total_taxes;//+ $cart->shipping_tax_total;

		
		
		$cart->cart_contents_total = $cart_items_totals;
	...
	public function wcrp_modify_cart_subtotal( $cart_subtotal, $compound, $cart ){
		...
		$new_cart_subtotal = $bulk_cart_subtotal + $wcrp_cart_subtotal - $discount_amount;
	...
	public function wcrp_display_custom_cart_price($item_price, $cart_item, $cart_item_key) {  
			...
			$optedout = get_post_meta($product_id, '_wcrp_checkbox', true);
			if ($optedout === 'yes'){
					$item_price = wc_price($cart_item['data']->price); 
				} 
			if (!is_cart()){	
			...
			if ( $optedout != 'yes' ) {
				...
				$price_to_display = wc_price($old_price);
				
			}
            $original_price_to_display = wc_price($custom_price);
			if ( ($cart_item['quantity'] > 1) && ( $optedout != 'yes' ) ){
            $item_price = '<span class="wcrp_cart_price_2"><del>' . $original_price_to_display . '1</del> <ins>' . $price_to_display . '</ins></span>';
			} else {
...
<?php 		
		...
		if (!is_admin()){
			...
			return $price;
				...
				$price = '';
			return '<a class="initialism fadeandscale_open btn btn-success button" href="#fadeandscale">Check Prices</a>'; // button for modal
